...
function sourceAge(payload: HistoryPayload | null) {
  ...
  if (payload.basis === 'hourly' && typeof payload.sourceAgeHours === 'number') {
    ...
    return { label: hours < 1 ? 'updated within the hour' : `updated ${hours.toFixed(1)}h ago`, stale: hours > 6 };
  }
  if (payload.basis === 'daily' && typeof payload.sourceAgeDays === 'number') {
    const days = payload.sourceAgeDays;
    return { label: days < 1 ? 'current day' : `updated ${days.toFixed(0)}d ago`, stale: days > 2 };
  }
...
export function OperationsHistory() {
  ...
  return <section className="history-section" aria-labelledby="history-heading">
    <div className="history-heading-row">
      <div><span className="eyebrow">RECENT OPERATIONS</span><h2 id="history-heading">What changed?</h2></div>
    ...
    <div className="history-shell">
      <div className="history-meta">
        <div><strong>{payload?.windowLabel || 'Loading…'}</strong>{payload?.through && <span>Through {shortDate(payload.through)} PT</span>}</div>
        {payload && age.label && <span className={age.stale ? 'history-age stale' : 'history-age'}>{age.label}</span>}
      </div>
      ...
      <div className="chart-card">
        {loading ? <p className="muted">Loading…</p> : payload?.error ? <p className="muted">{payload.error}</p> : chart && selectedSeries ? <>
          ...
          <svg viewBox="0 0 800 220" role="img" aria-labelledby="history-chart-title history-chart-desc">
            ...
            <desc id="history-chart-desc">Minimum {chart.min.toFixed(2)} and maximum {chart.max.toFixed(2)} {selectedSeries.unit} through {shortDate(payload?.through ?? null)}.</desc>
      ...
      <div className="insight-block" aria-live="polite">
        <div className="insight-heading"><span className="eyebrow">CHANGES</span></div>
        <div className="insight-grid">
          {payload?.insights.length ? payload.insights.map(insight => <article key={insight.id}><span>CHANGE</span><p>{insight.text}</p></article>) : <article><span>CHANGE</span><p>No meaningful change in this range.</p></article>}
        </div>
      </div>
    </div>
  </section>;
