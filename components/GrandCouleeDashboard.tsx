...
export function GrandCouleeDashboard({ initialStatus }: Props) {
  ...
  const outflowContextSub = status.recentRiver.currentOutflowVs7dPct !== null && status.recentRiver.outflow7dAverageKcfs !== null
    ? `${status.recentRiver.currentOutflowVs7dPct > 0 ? '+' : ''}${n(status.recentRiver.currentOutflowVs7dPct, 0)}% vs 7-day avg · ${n(status.recentRiver.outflow7dAverageKcfs, 1)} kcfs avg`
    : 'Columbia River flow below the dam';

  const dailyDifference = status.recentRiver.dailyInflowMinusOutflowKcfs;
  const inflowSub = dailyDifference === null
    ? 'Latest daily average into Grand Coulee'
    : Math.abs(dailyDifference) < 0.5
      ? 'Very close to latest daily outflow'
      : `${n(Math.abs(dailyDifference), 1)} kcfs ${dailyDifference > 0 ? 'above' : 'below'} latest daily outflow`;
  ...
  return <main>
    <header className="hero">
      ...
      <section className="metric-grid" aria-label="Current Grand Coulee conditions">
        ...
        <Metric label="Current outflow" value={status.flow.totalOutflowKcfs === null ? '—' : `${n(status.flow.totalOutflowKcfs, 1)} kcfs`} sub={outflowContextSub} tag="MEASURED" />
        <Metric label="Daily inflow" value={status.riverContext.inflowKcfs === null ? '—' : `${n(status.riverContext.inflowKcfs, 1)} kcfs`} sub={inflowSub} tag="REPORTED" />
        <Metric label="Lake change · 7d" value={status.recentRiver.lake7dChangeFt === null ? '—' : signed(status.recentRiver.lake7dChangeFt, ' ft')} sub="Compared with the reported lake level 7 days earlier" tag="DERIVED" />
    ...
    <section className="faq-section"><span className="eyebrow">QUICK ANSWERS</span><h2>Before you go</h2>
      ...
      <details><summary>Is Lake Roosevelt rising or falling today?</summary><p>{status.reservoir.change24hFt === null ? 'A 24-hour lake change is not available right now.' : `Lake Roosevelt is ${Math.abs(status.reservoir.change24hFt) < 0.01 ? 'essentially steady' : status.reservoir.change24hFt > 0 ? `up ${n(Math.abs(status.reservoir.change24hFt),2)} feet` : `down ${n(Math.abs(status.reservoir.change24hFt),2)} feet`} over the past 24 hours.${status.recentRiver.lake7dChangeFt !== null ? ` Compared with seven days ago, it is ${status.recentRiver.lake7dChangeFt > 0 ? 'up' : status.recentRiver.lake7dChangeFt < 0 ? 'down' : 'unchanged'} ${n(Math.abs(status.recentRiver.lake7dChangeFt),2)} feet.` : ''}`}</p></details>
      <details><summary>How much water is moving through Grand Coulee right now?</summary><p>{status.flow.totalOutflowKcfs === null ? 'No current outflow reading.' : `Current Columbia River outflow below Grand Coulee is ${n(status.flow.totalOutflowKcfs,1)} kcfs.${status.recentRiver.outflow7dAverageKcfs !== null && status.recentRiver.currentOutflowVs7dPct !== null ? ` That is ${n(Math.abs(status.recentRiver.currentOutflowVs7dPct),0)}% ${status.recentRiver.currentOutflowVs7dPct > 0 ? 'above' : status.recentRiver.currentOutflowVs7dPct < 0 ? 'below' : 'at'} the recent 7-day average of ${n(status.recentRiver.outflow7dAverageKcfs,1)} kcfs.` : ''}`}</p></details>
