  ...
  return <main>
    <header className="hero">
      ...
      <div className="hero-inner">
        <div className="hero-copy"><span className="eyebrow">LIVE INFRASTRUCTURE · GRAND COULEE, WASHINGTON</span><h1>GRAND COULEE <em>LIVE</em></h1><p className="lead">Start here, then look up at the dam.</p><p className="support">See what the river and reservoir are doing, what’s open today and when to be here. Then use the live dam view to make sense of what’s in front of you.</p><div className={`freshness ${telemetryFreshnessClass}`} title={status.telemetry.missingCoreSeries.length ? `Missing: ${status.telemetry.missingCoreSeries.join(', ')}` : 'All core operational fields are available'}><span className="pulse"/>{telemetryLabel} · {updated}</div></div>
        <div className="decision-card"><span className="eyebrow">IF YOU’RE COMING TODAY</span><h2>{status.decision.headline}</h2><p>{status.decision.detail}</p><div className="decision-weather">{status.weather ? `${n(status.weather.temperatureF,0)}°F · ${status.weather.shortForecast}` : 'Weather temporarily unavailable'}</div></div>
      </div>
