...
export function GrandCouleeDashboard({ initialStatus }: Props) {
  ...
  return <main>
    ...
    <section className="explain-section"><span className="eyebrow">HOW TO READ THIS TOOL</span><h2>Measured when published. Estimated only when explicitly labeled.</h2><div className="explain-grid"><article><h3>Operational telemetry is field-by-field</h3><p>Grand Coulee Live reads the USACE CWMS series independently. A current forebay or outflow value does not cause a missing spill, generation-flow or tailwater value to be treated as zero.</p></article><article><h3>Generation uses physics + calibration</h3><p>Estimated generation requires turbine flow and hydraulic head. The model is calibrated against reported Grand Coulee generation and is withheld whenever current turbine-flow telemetry is absent.</p></article><article><h3>Tailwater has a cautious fallback</h3><p>If measured tailwater is missing, Engineering Mode may show a low-confidence estimate from the official USACE Water Control Manual rating curve. Historical validation across 367 daily observations produced 0.31 ft MAE and 0.85 ft 95th-percentile absolute error, while USACE still notes that Rufus Woods Lake backwater can affect actual tailwater.</p></article><article><h3>Visitor times are reported</h3><p>Visitor Center hours, tours and the laser show come from current Bureau of Reclamation visitor information. Future years are not assumed.</p></article></div></section>
